...
#[cfg(not(windows))]
#[test]
fn task_queue_compaction_on_empty_queue() {
    let (index_scheduler, mut handle) = IndexScheduler::test(true, vec![]);

    // Register a compaction task with no finished tasks in the queue.
    index_scheduler.register(KindWithContent::TaskQueueCompaction, None, false).unwrap();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "registered_compaction");

    // The compaction should succeed (early exit path).
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "compaction_done");
}

#[cfg(not(windows))]
#[test]
fn task_queue_compaction_after_finished_tasks() {
    let (index_scheduler, mut handle) = IndexScheduler::test(true, vec![]);

    // Create some tasks that will finish.
    let (file0, documents_count0) = sample_documents(&index_scheduler, 0, 0);
    file0.persist().unwrap();

    index_scheduler
        .register(replace_document_import_task("doggos", None, 0, documents_count0), None, false)
        .unwrap();
    index_scheduler
        .register(
            KindWithContent::IndexCreation { index_uid: S("cattos"), primary_key: None },
    ...
    // Process both tasks so they become succeeded.
    handle.advance_one_successful_batch();
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_processing");

    // Now register a compaction task.
    index_scheduler.register(KindWithContent::TaskQueueCompaction, None, false).unwrap();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "compaction_registered");

    // Process the compaction. This should delete the finished tasks and compact the db.
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_compaction");
}

#[cfg(not(windows))]
#[test]
fn task_queue_compaction_does_not_delete_enqueued_tasks() {
    let (index_scheduler, mut handle) = IndexScheduler::test(true, vec![]);
    ...
    // Register more tasks but don't process them (they stay enqueued).
    index_scheduler
        .register(KindWithContent::DocumentClear { index_uid: S("doggos") }, None, false)
        .unwrap();

    // Register the compaction.
    index_scheduler.register(KindWithContent::TaskQueueCompaction, None, false).unwrap();

    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "before_compaction");

    // Compaction runs first (higher priority than normal tasks).
    // It should delete the succeeded index creation but keep the enqueued document clear.
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_compaction");

    // The enqueued document clear should still be processable.
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_document_clear");
}

#[cfg(not(windows))]
#[test]
fn task_queue_compaction_priority_over_normal_tasks() {
    let (index_scheduler, mut handle) = IndexScheduler::test(true, vec![]);

    // Enqueue a normal task and a compaction task. Compaction should run first.
    index_scheduler
        .register(
            KindWithContent::IndexCreation { index_uid: S("doggos"), primary_key: None },
            None,
    ...
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "both_registered");

    // First batch should be the compaction (higher priority).
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_first_batch");

    // Second batch is the index creation.
    handle.advance_one_successful_batch();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "after_second_batch");
}
